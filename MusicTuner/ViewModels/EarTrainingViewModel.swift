...
//  MusicTuner
//
//  ViewModel for Ear Training gamified learning module
//  Manages state machine: idle, teaching, quizzing, completed
//  Mirrors ChordMasteryViewModel pattern
//
...
enum EarTrainingMasteryState: Equatable {
    case idle
    ...
    static func == (lhs: EarTrainingMasteryState, rhs: EarTrainingMasteryState) -> Bool {
        switch (lhs, rhs) {
        case (.idle, .idle):
            return true
...
final class EarTrainingViewModel: ObservableObject {
    ...
    @Published private(set) var state: EarTrainingMasteryState = .idle
    ...
    func loadProgress() {
        currentUnlockedLevel = progressService.getEarTrainingUnlockedLevel()
        completedLevels = Set(EarTrainingCurriculum.levels.filter {
            progressService.isEarTrainingLevelCompleted($0.id)
        }.map { $0.id })
    }
    
    /// Check if a level is unlocked
    func isLevelUnlocked(_ level: LevelDefinition) -> Bool {
        return progressService.isEarTrainingLevelUnlocked(level.id)
    }
    ...
    /// Replay current quiz chord
    func replayQuizChord() {
        guard case .quizzing(_, _, let chord) = state else { return }
        chordEngine.playChord(chord)
    }
    ...
    private func startQuiz(for level: LevelDefinition) {
        ...
        if let firstQuestion = questions.first {
            ...
            // Auto-play the first quiz chord
            chordEngine.playChord(firstQuestion.correctChord)
        }
    }
    ...
    private func moveToNextQuestion(level: LevelDefinition, currentIndex: Int) {
        ...
        if nextIndex < questions.count {
            ...
            // Auto-play the next quiz chord
            chordEngine.playChord(nextQuestion.correctChord)
        } else {
            // Quiz complete
    ...
    private func finishQuiz(level: LevelDefinition) {
        ...
        if passed {
            ...
            progressService.markEarTrainingLevelCompleted(level.id)
            completedLevels.insert(level.id)
            
            // Unlock next level
            if let nextLevel = EarTrainingCurriculum.nextLevel(after: level) {
                progressService.saveEarTrainingUnlockedLevel(nextLevel.id)
                currentUnlockedLevel = progressService.getEarTrainingUnlockedLevel()
            }

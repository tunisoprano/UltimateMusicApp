...
struct EarTrainingLevelSelectView: View {
    ...
    private var completedCount: Int {
        EarTrainingCurriculum.levels.filter { viewModel.isLevelCompleted($0) }.count
    }

    private var progressSummary: some View {
        VStack(spacing: 8) {
            HStack {
                Text(String(format: L("levels_completed_format"), completedCount, EarTrainingCurriculum.totalLevels))
            ...
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    ...
                    Capsule()
                        ...
                        .frame(width: geo.size.width * CGFloat(completedCount) / CGFloat(max(EarTrainingCurriculum.totalLevels, 1)))
                }
            }
            .frame(height: 6)
        }
    ...
    private var currentLevel: LevelDefinition? {
        EarTrainingCurriculum.levels.first { viewModel.isLevelUnlocked($0) && !viewModel.isLevelCompleted($0) }
            ?? EarTrainingCurriculum.levels.last
    }
